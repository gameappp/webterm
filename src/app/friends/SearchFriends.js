...
const SearchFriends = () => {
  ...
  const searchHandler = () => {
    ...
    postData("/friends/search", { search })
      .then((res) => {
        setLoading(false);
  ...
  return (
    <div className="w-full flex flex-col gap-3 z-10 mb-3">
      <Toaster />

      <div className="flex items-center gap-1.5 -mb-1">
        <span className="text-sm font-medium text-gray-200">جست و جو</span>
      ...
      <div
        ...
        className={`relative w-full rounded-2xl transition-all duration-300 ${
          data?.result ? "max-h-96" : "max-h-12"
        }`}
      >
        <div className="relative w-full rounded-2xl p-[1px] bg-gradient-to-b from-blueColor/30 via-blueColor/10 to-transparent">
          <div className="relative w-full rounded-2xl bg-secondaryDarkTheme/80 backdrop-blur-sm p-3">
            <Input
              type="text"
              startContent={
                <button onClick={searchHandler} className="text-gray-400 hover:text-blueColor transition-colors">
                  <svg
              ...
              endContent={
                loading ? (
                  ...
                  <button onClick={searchClearHandler} className="text-gray-400 hover:text-danger transition-colors">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
        ...
        {data?.result && (
          <div className="w-full flex flex-col gap-3 p-4 border-t border-white/5 bg-secondaryDarkTheme/50 mt-1">
